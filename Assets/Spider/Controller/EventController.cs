...
public class EventController : SpiderController<EventController>, IController {
		...
		public bool QueueEvent(GameEvent evt) {
			if (!delegates.ContainsKey(evt.GetType())) {
				Debug.LogWarning("EventController: QueueEvent failed due to no listeners for event: " + evt.GetType());
				return false;
			}
